...
class MjViewerKeyboard(MjViewerBasic):
    """
    Extends :class:`.MjViewerBasic` to add video recording, interactive time and interaction controls.
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        # if action == glfw.PRESS or action == glfw.REPEAT: # Movement
        if action == glfw.PRESS:
            if glfw.get_key(window, glfw.KEY_KP_8): # move forward
                self.action[1] = 1.
            if glfw.get_key(window, glfw.KEY_KP_6): # move right
                self.action[0] = 1.
            if glfw.get_key(window, glfw.KEY_KP_2): # move down
                self.action[1] = -1.
            if glfw.get_key(window, glfw.KEY_KP_4): # move left
                self.action[0] = -1.
            if glfw.get_key(window, glfw.KEY_KP_9): # move up
                self.action[2] = 1.
            if glfw.get_key(window, glfw.KEY_KP_7): # move down
                self.action[2] = -1.
            if glfw.get_key(window, glfw.KEY_KP_0): # open/close hand
                if self.action[3] == 0:
                    self.action[3] = 1.
                else:
                    self.action[3] *= -1.
        if action == glfw.RELEASE: # No movement
            if key == glfw.KEY_KP_8:
                self.action[1] = 0.
            if key == glfw.KEY_KP_6: # move right
                self.action[0] = 0.
            if key == glfw.KEY_KP_2: # move down
                self.action[1] = 0.
            if key == glfw.KEY_KP_4: # move left
                self.action[0] = 0.
            if key == glfw.KEY_KP_9: # move up
                self.action[2] = 0.
            if key == glfw.KEY_KP_7: # move down
                self.action[2] = 0.
        if action != glfw.RELEASE:
            return
        elif key == glfw.KEY_TAB:  # Switches cameras.
...
class MjViewerController(MjViewerBasic):
    """
    Extends :class:`.MjViewerBasic` to add video recording, interactive time and interaction controls.
    ...
    def joystick_callback(self, jid):
        ...
        if present == 1:
            ...
            if abs(ax[0]) > 0.005: # account for some drift
                self.action[0] = ax[0]
            if abs(ax[1]) > 0.005:
                self.action[1] = ax[1]
            if abs(ax[4]) > 0.005:
                self.action[2] = ax[4]
            butt, n_butt = glfw.get_joystick_buttons(id)
            if butt[0] == 1:
                self.action[3] = 1.
            if butt[1] == 1:
                self.action[3] = -1.
    ...
    def key_callback(self, window, key, scancode, action, mods):
        self.joystick_callback(glfw.JOYSTICK_1) # assume only 1 controll
        if action != glfw.RELEASE:
            return
        elif key == glfw.KEY_TAB:  # Switches cameras.
            self.cam.fixedcamid += 1
